...
| 2026-09-12 | **v2d（Phase 1 轉換器 A 落地 + 全電池 11/11 PASS）**：Chat ↔ Responses 翻譯器——`chatToResponsesRequest` / `responsesObjectToChatResponse` / `pipeResponsesAsChat` / `pipeResponsesAsAnthropic`（+520 行）。chat/messages 路由 Spark 分支取代 B2 擋截，muse-spark* 三端點全通。風險 6 實測解除（stateless 回放 200、不需 reasoning_bridge）；tool_choice 僅支援 auto 降級設計；lazy-header 修正。驗證：T1 sync / T2 stream / T3 tool+降級 / T4 stream tool / T5 messages sync / T6 messages stream / T7 messages tool / T8 多輪 tool loop / REG mimo×chat / REG spark×responses B1 / REG mimo×responses 400 = 11/11 PASS。提交 `b1c37a0` | Mercury |
| 2026-09-12 | **v2e（Phase 1 代碼逐行重審 + B-Q1..Q5 修復）**：確認外部修正已入 `b1c37a0`（Q2 = sendHead 僅在發射分支提交 headers、Q3 = P2.8 補齊三路 `validateGenParams` 含 `max_output_tokens`、TAG/netCause 觀測埋點、中文超時訊息）。重審發現並修復 5 項：**B-Q1** 串流 chunk id 每塊重生成 → 改全程單一 id（OpenAI 客戶端按 id 分組重組）；**B-Q2** 串流完成塊丟棄 usage → `response.completed` usage 映射進 finish chunk；**B-Q3** 推理熔斷未計工具進展 → `toolCount === 0` 條件補上（長思考後合法 tool_call 不再被誤殺）；**B-Q4** `max_completion_tokens` 在 Spark 路徑被丟棄 → 譯為 `max_tokens` fallback；**B-Q5** 跨 TCP chunk 的錯誤 JSON 會靜默變空 200 → 累積原文尾部 16KB 作後備偵測（兩管線共用 `reportInlineError`）。驗證 `test_bq_fixes.mjs` 5/5 PASS（id 唯一性 + usage 透傳 + 兩路可用性 + native 回歸） | Mercury |
| 2026-09-12 | **v2f（外部發現 R1 = 翻譯路終結事件冪等，複查通過）**：用戶方發現 `pipeResponsesAsChat`/`pipeResponsesAsAnthropic` 的 `response.completed/incomplete/failed` 分支無冪等守衛——上游若如 chat SSE 重發終結事件，會 double-emit finish/[DONE]/message_stop（原生 chat 路有 `finishHandled`，翻譯路只有供 ABORT 判斷用的 `completed` 旗標）。修復 `0203d4c`：終結分支整體加 `&& !completed`，首個終結事件勝出；reasoning-cap 路徑由頂部 `stopped` 守衛覆蓋，ABORT 判斷不受影響。複查：`node --check` ✅ + Spark×chat 串流實測 finish chunks=1 / [DONE]=1（單終結）✅ | Mercury |
